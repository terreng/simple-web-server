...
lang = {
    ...
    "zh_CN": {
        ...
        "option_rewriteTo": "Rewrite to (for SPAs)",
        "option_rewriteTo_description": "如果启用了单页重写选项，请指定用来重写的文件。参见 <a href=\"https://developer.mozilla.org/zh-CN/docs/Glossary/SPA\">单页应用</a>.",
        "option_directoryListing": "显示目录列表",
        ...
        "option_cors": "Set CORS headers",
        "option_cors_description": "允许跨域请求。 Sets <code>Access-Control-Allow-Origin</code> header to <code>*</code>, <code>Access-Control-Allow-Methods</code> to <code>GET, POST, PUT, DELETE</code>, and <code>Access-Control-Max-Age</code> to <code>120</code>. <a href=\"https://developer.mozilla.org/zh-CN/docs/Web/HTTP/CORS\">了解更多有关 Cross-Origin Resource Sharing.</a>",
        "option_hiddenDotFiles": "提供隐藏/.文件的访问",
        "option_hiddenDotFiles_description": "允许请求隐藏/.文件。这些文件或文件夹的名称以.字符开头。注意：要在目录列表中显示它们请开启 \"在目录列表中显示隐藏的/.文件\"",
        ...
        "option_staticDirectoryListing": "始终使用静态目录列表",
        "option_staticDirectoryListing_description": "禁用目录列表页的JavaScript增强。",
        "option_hiddenDotFilesDirectoryListing": "在目录列表中显示隐藏的/.文件",
        "option_hiddenDotFilesDirectoryListing_description": "如果启用了提供隐藏/.文件的访问，并勾选此选项，则它们会显示在目录列表中，其中包括<code>.swshtaccess</code>文件。",
        ...
        "option_customErrorReplaceString_description": "(可选)指定将在错误页面中查找并替换为当前路径的自定义字符串。 用例，您的自定义404页面内容中包含：<code>\"位于{{PATH}}的文件不存在\"</code>这样的字符串，并且您将此选项设置为<code>{{PATH}}</code>(默认)，则当您的自定义页面被提供时，它将显示<code>位于/example.txt的文件不不存在</code>。",
        "section_security": "安全",
        "option_https": "使用 HTTPS",
        "option_https_description": "使服务器只能通过安全连接(HTTPS)而不是HTTP访问。使用相同的单个端口，因此您不能同时同时使用HTTP和HTTPS。 参见 <a href=\"https://simplewebserver.org/docs/https.html\">使用 HTTPS/a>.",
        "option_https_abbreviation": "HTTPS",
        ...
        "option_httpAuth_description": "需要使用HTTP基本身份验证协议进行身份验证。在HTTP基本身份验证用户名和HTTP基本身份验证密码选项中指定用户名和密码。如果用户名或密码选项缺失或无效，则Web服务器将无法访问。",
