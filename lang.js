...
lang = {
    ...
    "ru": {
        ...
        "setting_tray": "Показывать значёк в трее или меню",
        "setting_tray_macos": "Показывать значёк в меню",
        "setting_tray_windows": "Показывать значёк в трее",
        "setting_tray_description": "When enabled, adds a shortcut icon to the system tray or menu bar area whenever Simple Web Server is running.",
        ...
        "add_plugin_confirm_description": "Устанавливайте плагины только от тех кому доверяте.<br><br>Плагины не изолированы и имеют тот же доступ что и это приложение.",
        ...
        "option_path_description": "Папка в котрой лежат файлы самого сайта. Если сама папка внутри скрытой папки тогда убедитесь что включили опцию <a href=\"https://simplewebserver.org/docs/options.html#serve-hidden-dot-files\">Показывать скрытые файлы</a>.",
        "choose_folder": "Выберете папку",
        "path_missing": "Пожалуйста выберете путь к папке",
        "option_port": "Порт",
        "option_port_description": "Номер от 1 до 65535. Стандартный порт 80 или 443 для HTTPS но они могут быть уже заняты или недоступны. Часто используют 8000, 8080, 8443. Открыть в броузере сайт можно будет по адресу <code>http://localhost:[ПОРТ]</code>.",
        ...
        "option_spa_description": "Automatically rewrite all paths that don't exist to a single page. For <a href=\"https://developer.mozilla.org/en-US/docs/Glossary/SPA\">Single Page Applications</a>.",
        "option_rewriteTo": "Перенаправлять запросы на страницу...",
        "option_rewriteTo_description": "Если перенаправление для одностраничных сайтов SPA включено то укажите страницу куда перенапрвлять.",
        ...
        "option_excludeDotHtml": "Исключить расширение .html",
        "option_excludeDotHtml_description": "Exclude .htm and .html extensions from URLs. For example, <code>/example.html</code> will redirect to <code>/example</code>. If a file exists at the path without an extension, the HTML file will still be rendered instead.",
        ...
        "option_cacheControl_description": "Optionally specify a custom <code>Cache-Control</code> HTTP header value. <a href=\"https://developer.mozilla.org/ru/docs/Web/HTTP/Headers/Cache-Control\">Learn more about the Cache-Control header.</a>",
        "option_cors": "Установать заголовки CORS",
        ...
        "option_staticDirectoryListing_description": "Уберает JavaScript улучшения для сгенерированных страниц с списком файлов в папке.",
        ...
        "option_htaccess_description": "Вы можете использовать файлы <code>.swshtaccess</code> чтобы установить правила конкретным папкам. Подробнее в <a href=\"https://simplewebserver.org/docs/swsaccess.html\">Продвинтуая конфигурация с помощью файла .swshtaccess</a>.<br><br>Эта функциональсть эксперементальная и может менятся в будущих версиях.",
        "section_error_pages": "Страницы ошибок",
        "option_custom404": "Страница ошибки 404",
        "option_custom404_description": "*.html файл со страницей 404 page. Если файла нет то будет использоватся стандартная по умолчанию",
        "option_custom403": "Страница ошибки 403",
        "option_custom403_description": "*.html файл со страницей 403 page. Если файла нет то будет использоватся стандартная по умолчанию",
        "option_custom401": "Страница ошибки 401",
        "option_custom401_description": "*.html файл со страницей 401 page. Если файла нет то будет использоватся стандартная по умолчанию",
        "option_customErrorReplaceString": "Переменная окружения с файлом",
        "option_customErrorReplaceString_description": "Опционально укажите строку вместо которой следует вставить теущий путь к файлу на котором возникла ошибка. Например, если в странице 404 написано: <code>Файл {{PATH}} не существует</code> и вы установили эту опцию в <code>{{PATH}}</code>, тогда в пользователь увидит <code>Файл /example.txt не существует</code>.",
        ...
        "option_httpsKey_description": "Если вы хотите использовать существующий привтаный ключ TLS (т.е. privkey.pem) то вставьте его текст сюда. Подробнее про <a href=\"https://simplewebserver.org/docs/https.html\">Использование HTTPS</a>.",
        "generate_crypto": "Сгенерировать сертификат-заглушку",
        "generate_crypto_overwrite": "Перетереть сертификат и приватный ключ?",
        "generate_crypto_overwrite_description": "Уже есть сертификат и привтный ключ. Это действите перетрёт их.",
        ...
        "httpAuthUsername_invalid": "Имя пользователя не может содаржать \":\"",
